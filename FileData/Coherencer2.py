...
class Coherencer(object):
  ...
  ***Argument names are designed to be illustrative and may be different but
  ...
  def loadDB(self, path="pkb_matrix_all_new"):
  ...
  def setParams(self, queries=['party'], numTerms=5, threshold=0.24,
                matrixDegree=2):
    ...
    considered coherent; 0.37 was tested to be best with Peekaboom
    ...
    #Interesting... In original run, duplicates in the coTerms were possible.
    #This additionally stacks probability of selection for high co-occur terms.
    self.coTerms = list(set(self.coTerms))
  ...
  def askCycle(self, nTerms, t, seed, num=False):
    ...
    for term in self.termsToProc:
      ...
      #To seed with topN
      if seed:
        self.selTerms.extend(self.topN(term, nTerms-1))
      results.append(self.ask(False))
      avgs.append(self.totalAvg)
      self.reset()
    return results, avgs

  def run(self, nTerms, term, t, seed=True):
    self.setParams([term], numTerms=nTerms, threshold=t)
    if seed:
      terms = self.topN(term, nTerms-1)
      self.selTerms.extend(terms)
      for trm in terms:
        self.coTerms.remove(trm)
    return self.ask(False)

  def runLoop(self, nTerms, term, t, seed=True):
    a = False
    inc = 0
    while a == False and inc < 10:
      a = self.run(nTerms, term, t, seed)
      self.reset()
      inc += 1
    return a
...
class Comparer(object):
  ...
  ***Argument names are designed to be illustrative and may be different but
  ...
  def loadDB(self, path=os.path.join("D:\\", "pkb_by_lbl_newest")):
