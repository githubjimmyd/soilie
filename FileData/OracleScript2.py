...
def proximity(query):
    ...
    temppage = urllib.urlopen("http://campuscurico.utalca.cl/~castudillo/research/pkb/co_ocurrence/proximity-action.php", tempparams)
    tempstring = ''
    #print temppage.read()
    tempstring = tempstring + temppage.read()
    position = tempstring.find("<pre>")
    stop = tempstring.find("</pre>")
    results = tempstring[position+5:stop]
    results = results.split()
    number = 1
    resultsDict = dict()
    while number < len(results)-1:
        resultsDict[results[number]] = results[number+1]
        number += 3
    '''
    tempstring = strip_html(tempstring)
