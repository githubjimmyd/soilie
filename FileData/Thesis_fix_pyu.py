...
class Coherencer(object):
  ...
  ***Argument names are designed to be illustrative and may be different but
  ...
  """
  def __init__(self, path='pkb_matrix_sept2013-2'):
...
##    self.filterType = None
  ...
  def setParams(self, queries=['party'], numTerms=5, threshold=0.5, seed=True):
...
##    self.sThreshold = sThreshold
  ...
  def assess(self):
    ...
    if total > self.best[0]:
      self.best = (total, [x for x in self.selTerms])
...
##      r = True
    ...
    for index in range(len(self.selTerms)):
      if vals[index][0] in self.queries:
        pass
      else:
        self.selTerms.remove(vals[index][0])
        break
    try:
...
##          print self.selTerms
  ...
  def runLoop(self, num, query, t=0.5):
  ...
  def askCycle(self, numTerms=5, threshold=0.5, #sThreshold=10,
               seed=True, num=False):
...
#assumes that the models are setup and their terms to proc are identical
def pairedTestNonstat(compare, func1, func2, reset1, reset2, nruns1):
  r1 = []
  for _ in range(nruns1):
    reset1()
    r1.append(func1())
  r1s = []
  for x in r1:
    r1s.append([compare(y) for y in x])
  r2 = func2()
  r2s = [compare(x) for x in r2]
  totals = []
  for x in r1s:
    bothcor = sum([True for i, y in enumerate(x) if r2s[i] and y])
    bothfail = sum([True for i, y in enumerate(x) if y == False and r2s[i] == False])
    cohcor = sum([True for i, y in enumerate(x) if y == True and r2s[i] == False])
    topcor = sum([True for i, y in enumerate(x) if y == False and r2s[i] == True])
    totals.append([bothcor, bothfail, cohcor, topcor])
  return r1, r1s, r2, r2s, totals

def pairedTestStat(compare, func1, func2, reset1, reset2, nruns1):
  r1 = []
  r2 = []
  for k in range(nruns1):
    if k%10 == 0:
      print k
    reset1()
    reset2()
  ...
  for x in r1:
    r1s.append([compare(y) for y in x])
  for x in r2:
    r2s.append([compare(y) for y in x])

  totals = []
  for j in range(nruns1):
    bothcor = sum([True for i in range(8372) if r2s[j][i] and r1s[j][i]])
    bothfail = sum([True for i in range(8372) if r1s[j][i] == False and r2s[j][i] == False])
    cohcor = sum([True for i in range(8372) if r1s[j][i] == True and r2s[j][i] == False])
    topcor = sum([True for i in range(8372) if r1s[j][i] == False and r2s[j][i] == True])
    totals.append([bothcor, bothfail, cohcor, topcor])
  return r1, r1s, r2, r2s, totals
