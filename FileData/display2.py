...
def cut_out_background(background, img_and_objs):
    ...
    for object_ in img_and_objs[1]:
        vertices = get_vertices(img_and_objs[0], object_)
        box = bounding_box(vertices)
...
def display_from_file(filename, num, mask=False, cut=False):
...
##            imagine(item, position, draw=doingGrabcut, color=colors[i])
    ...
    if img_and_objs == []:
        ...
        if cut:
            background_mask = cut_out_background(background, img_and_objs)
    imagine(objects[0], main_position, draw=False, color=colors[0])
    for i, object_ in enumerate(objects[1:]):
        angle, distance = angledist[i]
        position = find_position(angle, distance, main_position)
        imagine(object_, position, draw=doingGrabcut, color=colors[i+1])
    if mask:
        i = make_everything(background, background_mask)
        os.chdir('temp'+str(i))
        subprocess.call(["InpaintImageMain.exe"])
    else:
        draw_everything()

def make_everything(background, background_mask):
    ...
    while True:
        try:
            ...
            break
    pygame.image.save(background, os.path.join('temp'+str(i), 'background.jpg'))
    pygame.image.save(background_mask,
                      os.path.join('temp'+str(i), 'background_mask.jpg'))
    image = pygame.Surface((width,height), pygame.SRCALPHA, 32)
    image2 = pygame.Surface((width,height), pygame.SRCALPHA, 32)
    image.fill(white)
    to_display.draw(image)
    pygame.image.save(image, os.path.join('temp'+str(i), 'source.jpg'))
    image2.fill(black)
    to_mask_display.draw(image2)
    pygame.image.save(image2, os.path.join('temp'+str(i), 'source_mask.jpg'))
    print to_mask_display.sprites()
