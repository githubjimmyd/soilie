...
if doingGrabcut:
    import cv2
if usingImagenet:
    import imagenetretriever
    
imagenetpattern = re.compile("[xy][minmax]+>[0-9]+</[xy][minmax]+")
...
class Imagined(pygame.sprite.Sprite):
    ...
    def __init__(self, colors, pixels, box, position, color=False, use_imagenet=True):
        pygame.sprite.Sprite.__init__(self)
...
def imagenet_filter(box):
    """ Simplified version of inside_polygon, for ImageNet files """
    pixels = []
    x = box[0]
    while x < box[2]:
        y = box[1]
...
def get_imagenet_box(annotation, nobb, image):
    """ Gets the bounding box from the annotation """
    #To do: check for identical sizes
    if nobb == "Found nobb":
        size = image.get_size()
        xmargin = int(size[0]/15.0)
        ymargin = int(size[1]/15.0)
        box = [0+xmargin,0+ymargin,size[0]-xmargin,size[1]-ymargin]
    else:    
        box_file = open(annotation,"r")
        box_text = box_file.read()
        almost_box = re.findall(imagenetpattern, box_text)[0:4]
        box = [eval(re.findall(numberpattern,number)[0]) for number in almost_box]
    return box

def get_imagenet_image(image_name):
    image = pygame.image.load(image_name)
    return image

def imagine(thing, position=(0.5,0.5), draw=True, grabcut=doingGrabcut, color=False, using_imagenet=True, using_pkbm=False):
    print(using_pkbm)
    ...
    if using_imagenet:
        things = ["usable"+thing+".xml","usable"+thing+".jpg"]
    else:
        if using_pkbm == False:
    ...
    if len(things) == 0:
        ...
        if using_imagenet == True or using_pkbm == False:
            image_name = things[1]
            annotation = things[0]
        
        if using_imagenet:
            nobb = imagenetretriever.main(thing)
            if nobb == "Not found":
                return None
            image = get_imagenet_image(image_name)
            vertices = get_imagenet_box(annotation, nobb ,image)
        else:
            if using_pkbm == False:
                ...
                im = Image.open(image)
                x_min = 0
                x_max = x_min + im.size[0]
                y_min = 0
                y_max = y_min + im.size[1]
                vertices = [[x_min, y_min], [x_min, y_max], [x_max, y_min], [x_max, y_max]]
        ...
        if grabcut:
            ...
            if using_imagenet:
                pixels, box = imagenet_filter(vertices)
            else:
                print(vertices)
                pixels, box = inside_polygon(vertices)
...
def display_from_file(filename, num=1000, mask=False, painter=False, from_imagenet=True, from_pkbm=False):
    print(from_pkbm)
    ...
    if from_imagenet:
        ...
        main_object = filename
    main_position = [0.5,0.5]
    if mask:
        colors = make_mask_colors(num)
    else:
    ...
    if from_imagenet:
        for i, line in enumerate(f):
            if line[0]== "#":
                ...
                imagine(item, position, draw=False, color=colors[i])
    else:
        #print("colours: " + str(len(colors)))
        #print(colors)
        imagine(main_object, main_position, draw=False, color=colors[0], using_imagenet=from_imagenet, using_pkbm=from_pkbm)
    ...
    if mask:
        ...
        if painter:
            ...
            image.fill(white)
            to_display.draw(image)
            pygame.image.save(image, os.path.join("imagining","sketch.jpg"))
        else:
            draw_everything()
...
def make_everything():
    ...
    while True:
        try:
            ...
            break
    image = pygame.Surface((width,height), pygame.SRCALPHA, 32)
    image2 = pygame.Surface((width,height), pygame.SRCALPHA, 32)
    image.fill(white)
    to_display.draw(image)
    pygame.image.save(image, os.path.join('temp'+str(i), 'main_image.jpg'))
    image2.fill(white)
    to_mask_display.draw(image2)
    pygame.image.save(image2, os.path.join('temp'+str(i), 'mask_image.jpg'))
