...
def split_concept(tree):
    ...
    if not subconcept:
        print "Concept not found and cannot be broken down any further!\nConcept '" + tree.get_name() + "' not found"
    else:
...
def combine_concept(concepts):
    if not isinstance(concepts, (list, tuple)):
        print "Invalid 'combine_concept' argument\n" + str(concepts) + " given"

    output(("combine ", concepts),1)
    count = len(concepts)
    if count < 1:
        print "No concepts to merge"
    elif count == 1:
        output("comb1", 2)
        return concepts[0]
    elif count == 2:
        ...
        print "Cannot combine concepts of greater then 3"
...
def search_candidates(c1, c2):
    ...
    if c1.lexical_category != 'noun':
        print str("Concept '" + c1.name + "' is of type '" + c1.lexical_category + "'\n Should be type 'noun'")
    ...
    if c1.WordNetName.lower() in N:
        ...
        print str("Concept '" + c1.name + "' is not a word in wordNet")
    ...
    for c in Prototype.get_all('preposition'):
        ...
        if rel_name == c.relation_name:
            ...
            if c.relation_left == left: score_left = 1
                ...
                if c.relation_left.lower() in N and noSenseLeft == 0:
            ...
            if c.relation_right == right: score_right = 1
                ...
                # score from 0 to 1 depending on how close it is semantically
                if c.relation_right.lower() in N and noSenseRight == 0: #and c.relation_right:
...
class Prototype:
    ...
    def has(c):
        ...
        # if c is a prototype
            ...
            print "Invalid concept class in has() function"
    ...
    def __init__(self, name, lexical_category="", preference=[], WordNetName="", LC_guess = True):
        ...
        if self.lexical_category == "preposition":
            if len(split_names) == 1:
                ...
                print self.name + "Not a valid relation"
...
class Exemplar:
    ...
    ##exemplar_list = []                    #@ Testing this here without saving the exemplar
    ...
    def __init__(self, name="", secondary_names=[], attributes = {}, lexical_category="", example_attributes={}, WordNetName="", preference="", LC_guess = True, mental_image=False):
        ...
        ##Exemplar.exemplar_list.append(self)                   #@ We'll see what happens.
...
def create_examples(filename):
    ...
    picklingfile = open("SaveData\save"+sys.argv[1]+".txt", "wb")
...
if __name__ == '__main__':
    ...
    MASTER_CONCEPT_LIST = open("Master_Concept_List_Vinc.txt", "rb")
    if DEBUGGING:
        pdb.set_trace()

    #print('Load a file?')
    #loadOrNot = input()
    loadOrNot = "n"    ## Are we loading a file
    if loadOrNot == "y":
        #print('Name of the file')
        picklingfile = open("..\SaveData\saveDataTest.txt", "rb")
        Prototype.concept_list = cPickle.load(picklingfile)
        picklingfile.close()
        i = 2
       # while (i < 25):
        #    picklingfile = open("..\SaveData\save"+str(i)+".txt", "rb")   ##To change the name of the file, change the input() to the name of the file you wish to load in between ""
         #   Prototype.concept_list.extend(cPickle.load(picklingfile))
          #  picklingfile.close()
           # print i
            #i += 1
    ...
    TRAINING = "n"						## Do we wish to train visuo on dataset y or n
    if TRAINING == "y":
        #print("Please input examplefile name.")
        EXAMPLE_FILE = "..\TrainData\Train"+sys.argv[1]+".txt"				## Input the name of the file containing said dataset
        #print('Would you like to get the output?')
        OUTPUT = "n"									## Do we want an output distribution?
    ...
    if OUTPUT == "y":
        ...
        print """Please insert query. Please use following format
        [an] above sky"""
        query = 'nose above ear'
        query = query.replace(",", " ")
        splittedquery = query.split()

        firstWord = splittedquery[0][1:-1]
        secondWord = splittedquery[2]

        loadedFiles = []
        firstWordLoaded = 0
        secondWordLoaded = 0
        firstFile = False
        reverseQuery = list(splittedquery)
        reverseQuery[0] = "["+splittedquery[2]+"]"
        reverseQuery[2] = splittedquery[0][1:-1]
        reverseQuery = " ".join(reverseQuery)
        perfectmatch = False

##        for lines in MASTER_CONCEPT_LIST:
##            splittedlines = lines.split()
##            splittedlines[1] = splittedlines[1][1:-1]
##            lines = " ".join(lines.split()[1::])
##            if query == lines or reverseQuery == lines:
##                loadedFiles.append(splittedlines[0])
##                print lines
##                pickleFile = open("..\SaveData\save"+splittedlines[0]+".txt", "rb")
##                Prototype.concept_list = cPickle.load(pickleFile)
##                pickleFile.close()
##                perfectmatch = True
##                break
##
##        MASTER_CONCEPT_LIST.seek(0)
##
##        if not perfectmatch:
##            for lines in MASTER_CONCEPT_LIST:
##                splittedlines = lines.split()
##                splittedlines[1] = splittedlines[1][1:-1]
##                if firstWord.upper() in splittedlines:
##                    if not firstFile:
##                        firstFile = True
##                        loadedFiles.append(splittedlines[0])
##                        print lines
...
##                        Prototype.concept_list = cPickle.load(pickleFile)
##                        pickleFile.close()
##                        firstWordLoaded = firstWordLoaded +1
##
##                    elif splittedlines[0] not in loadedFiles and firstWordLoaded < 21:
##                        loadedFiles.append(splittedlines[0])
##                        print lines
##                        pickleFile = open("..\SaveData\save"+splittedlines[0]+".txt", "rb")
##                        Prototype.concept_list.extend(cPickle.load(pickleFile))
##                        pickleFile.close()
##                        firstWordLoaded = firstWordLoaded +1
##
##                elif secondWord.upper() in splittedlines:
##                    if not firstFile:
##                        firstFile = True
##                        loadedFiles.append(splittedlines[0])
##                        print lines
...
##                        Prototype.concept_list = cPickle.load(pickleFile)
##                        pickleFile.close()
##                        secondWordLoaded = secondWordLoaded +1
##
##                    elif splittedlines[0] not in loadedFiles and secondWordLoaded < 21:
##                        loadedFiles.append(splittedlines[0])
##                        print lines
##                        pickleFile = open("..\SaveData\save"+splittedlines[0]+".txt", "rb")
##                        Prototype.concept_list.extend(cPickle.load(pickleFile))
...
##                        secondWordLoaded = secondWordLoaded +1
        ...
        for attrib in complete_concept.attributes:
            ...
            resultsToReturn.append(complete_concept.attributes[attrib].defuzzify())
            print
            #print complete_concept.attributes['size'].output()
            #print "Defuzz value:", complete_concept.attributes['size'].defuzzify()

        OutPutFile = open("FromVisuoToDisplayTEST"+firstWord+".txt", "r+")
        OutPutFile.seek(0,2)
        OutPutFile.write(splittedquery[0][1:-1].lower()+"-"+splittedquery[2].lower()+" "+str(resultsToReturn[0])+" "+str(resultsToReturn[1])+" \n")#ex: an-sky 102.3 0.34
        OutPutFile.close()
